...
def InOutPaddings(x: torch.Tensor):
    ...
    paddingInput = nn.ReflectionPad2d(
        padding=(
            padding_width // 2,
    ...
    paddingOutput = nn.ReflectionPad2d(
        padding=(
            0 - padding_width // 2,
            padding_width // 2 - padding_width,
            0 - padding_height // 2,
...
class UpConvNorm(nn.Module):
    def __init__(
        self, in_channels: int, out_channels: int, mode="transpose", norm=False
    ):
        super().__init__()
        ...
        if mode == "transpose":
            ...
            self.upconv = nn.Sequential(
                ConvNorm(
                    in_channels, 4 * out_channels, kernel_size=3, stride=1, norm=norm
                ),
                PixelShuffle(2),
            )
        else:
...
class meanShift(nn.Module):
    def __init__(self, rgbRange, rgbMean, sign, nChannel=3):
        ...
        if nChannel == 1:
            ...
            self.shifter.bias.data = torch.Tensor([r, g, b])  # type: ignore
        else:
            r = rgbMean[0] * rgbRange * float(sign)
            g = rgbMean[1] * rgbRange * float(sign)
            b = rgbMean[2] * rgbRange * float(sign)
            ...
            self.shifter.bias.data = torch.Tensor([r, g, b, r, g, b])  # type: ignore
...
class RCAB(nn.Module):
    def __init__(
        ...
        self.body = nn.Sequential(
            ConvNorm(
                in_feat, out_feat, kernel_size, stride=2 if downscale else 1, norm=norm
            ),
            act,
...
class ResidualGroup(nn.Module):
    def __init__(
        ...
        modules_body = [
            Block(n_feat, n_feat, kernel_size, reduction, bias=True, norm=norm, act=act)
...
def pixel_shuffle(input: torch.Tensor, scale_factor: float):
...
def conv(
    in_channels: int, out_channels: int, kernel_size, stride=1, bias=True, groups=1
):
    return nn.Conv2d(
        in_channels,
        out_channels,
...
def conv3x3(
    in_channels: int, out_channels: int, stride=1, padding=1, bias=True, groups=1
):
    return nn.Conv2d(
        in_channels,
        out_channels,
...
def conv7x7(
    in_channels: int, out_channels: int, stride=1, padding=3, bias=True, groups=1
):
    return nn.Conv2d(
        in_channels,
        out_channels,
...
class Interpolation_res(nn.Module):
    def __init__(
        self, n_resgroups, n_resblocks, n_feats, act=nn.LeakyReLU(0.2, True), norm=False
    ):
        super().__init__()
