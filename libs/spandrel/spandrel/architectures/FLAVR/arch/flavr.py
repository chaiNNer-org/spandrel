...
JoinType = Literal["concat", "add", "first"]


def joinTensors(X1, X2, type: JoinType = "concat"):
    if type == "concat":
...
class UNet_3D_3D(nn.Module):
    ...
    def __init__(
        ...
        self.joinType: JoinType = joinType
        ...
        if n_outputs > 1:
            resnet_3d.useBias = True

        if block == "unet_18":
            self.encoder = resnet_3d.unet_18(bn=batchnorm)
        elif block == "unet_34":
            self.encoder = resnet_3d.unet_34(bn=batchnorm)
        else:
            raise ValueError(f"Block {block} not recognized")

        self.decoder = nn.Sequential(
            Conv_3d(
                nf[0], nf[1], kernel_size=3, padding=1, bias=True, batchnorm=batchnorm
            ),
            upConv3D(
                nf[1] * growth,
                nf[2],
